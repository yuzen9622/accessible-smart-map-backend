...
### 3.5 使用者逐字稿的 interim / final / correction（⚠️ 前端渲染規則）

`role:"user"` 的逐字稿分三種 frame，同一句話由 `utteranceId` 串起來（每個 session 內單調遞增，形如 `u1`、`u2`）：

- **interim**（`transcript`，`final` 不存在或為 `false`）：使用者說話中即時送出的部分逐字稿片段（fragment），未經校正，用來做即時字幕。同一句話會連續來多則，共用同一個 `utteranceId`。
- **final**（`transcript`，`final:true`）：整句結束時**立即**送出的完整整句，帶該句的 `utteranceId`。內容是**未校正**的原文。
- **correction**（`transcript.correction`）：後端用一次性 LLM 對台灣地名／車站／捷運／路線名做音近錯字校正（例如「珠北車站」→「竹北車站」），約在 final 之後 0.5–2.5 秒到，帶**同一個** `utteranceId`。**只有在校正結果與原文不同時才會送**；沒有需要改的地方、LLM 逾時或失敗時不送任何 frame（此時 final 的原文就是最終顯示內容）。

**為什麼拆成三種**：校正需要一次 LLM 往返。舊版把 final 壓在校正之後才送，於是 `final:true` 會落在模型逐字稿的中間，前端若以 role 變化切訊息，畫面就會出現「使用者訊息重複兩則 + 助理回覆被切成兩則」。現在 final 一律**先於**該回合的任何 `role:"model"` 逐字稿送出，校正只是事後的原地修正。

**前端必須這樣渲染**：

- `role:"user"` interim：把片段累加到 `utteranceId` 對應的字幕元素（不存在就新建）。
- `role:"user"` final：用該則 `text` 整句取代該 `utteranceId` 的元素，並結束這句（下一句會拿到新的 `utteranceId`）。
- `transcript.correction`：以 `utteranceId` 找回**已經渲染過**的那則使用者訊息，**原地取代**文字，不可 append 成新訊息。找不到（例如已捲離、已清空）就直接忽略。
...
前端                         後端 Voice Gateway              Gemini Live API
 ...
 │ ◀───transcript.correction─  │ (校正 LLM 回來，原地修正該句)   │  （只在有改動時送，可能落在此處）
